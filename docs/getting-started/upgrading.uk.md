...
Поки служба зупинена, скрипт ще й перевіряє, що юніт запускає uvicorn з `--loop asyncio`. Інсталяції, створені до того, як інсталятор почав його ставити (липень 2026), цього прапорця так і не отримали, а без нього сервер працює на uvloop, який може непомітно обрізати завантаження у віртуальний принтер. Простому однорядковому юніту uvicorn прапорець дописується (файл спершу копіюється поруч); юніт із drop-in-ами, скриптом-обгорткою чи перенесеним рядком команди лишається як є, і скрипт каже, що дописати руками. BamDude ще й пише попередження на старті, щойно опиняється на uvloop.
...
1. Спершу зробіть резервну копію (див. [Команди backup](#команди-backup)). Це єдиний крок, який ніхто за вас не зробить.
...
!!! warning "Рядки-сироти видаляються, і це пишеться в журнал"
...
!!! danger "Невдалий імпорт зупиняє старт, а не продовжує його"
...
!!! warning "Мажор PostgreSQL зафіксований"
...
!!! tip "Оновлюйтесь при зупиненому сервері"
...
- Для PostgreSQL в окремому контейнері є штатний override `docker-compose.postgres.yml`. Зверніть увагу на застереження про host-мережу в [Підтримці PostgreSQL](../features/postgresql.md): зі штатним `network_mode: host` BamDude не дістане інший контейнер за іменем сервісу Compose.
