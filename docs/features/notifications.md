...
| `print_progress` | At 25% / 50% / 75% progress. A global **minimum duration** setting (Settings → Notifications) can mute these for prints estimated shorter than N minutes — the duration is estimated from the printer's own remaining-time report at each milestone, and an unknown estimate sends rather than guesses away. `0` (the default) always sends. The floor **cascades**: every Telegram chat carries its own value (beside the Progress Milestones checkbox in the chat's settings) and every other provider carries its own (on its card, beside the same toggle); an empty value at either level inherits the global one, `0` = always send. |
