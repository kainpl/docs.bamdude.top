...
=== "Ланцюжок походження"
    ...
    | `source_content_hash` | SHA256 **непатченого** джерела. NULL, коли цей архів *і є* джерелом (патчі не застосовувались). |
    | `applied_patches` | JSON-список ідентифікаторів патчів, які пайплайн диспатчу застосував перед завантаженням, напр. `["mesh_mode_fast_check_off"]`. Інформативно; семантика "reprint-reapply" поки що не підключена. |
...
- Запити дедупу використовують `COALESCE(source_content_hash, content_hash)` -- патчені варіації колапсують проти свого оригіналу; зовнішні друки без джерела далі дедупляться за raw content-хешем (їх поведінка не змінилась).
- Reprint з наявного архіву копіює файл знову у свіжу директорію архіву -- новий `content_hash`, але той самий `source_content_hash`, тож історія reprint-ів лишається прив'язаною до оригінального дизайну.
- Зовнішні друки (стартовані з екрану принтера / з хмари / ручний старт зі SD) отримують один SELECT-запит при створенні архіву: якщо будь-який попередній архів на цьому принтері збігається за `content_hash` або `source_content_hash`, ланцюжок успадковується.

Сторінка Архівів виставляє фільтр "duplicates", який групує рядки за цим ефективним хешем. Запити статистики та лічильники print_count для файлів бібліотеки використовують те саме правило `COALESCE`.

!!! info "Міграція m009"
    `source_content_hash` та `applied_patches` додані в m009. Старі рядки лишаються NULL -- `COALESCE` відкочується на `content_hash`, тож поведінка для до-міграційних архівів не змінюється.
...
## :material-broom: Авточистка 3MF *(0.4.1)*
...
**Параметри → Друк → Налаштування архіву → "Авточистка збережених 3MF-файлів"**

- **Тогл** -- вимкнено за замовчуванням.
- **Поле зберігання** -- дні, мінімум 1, дефолт 30.
- **Live preview** -- показує, що почистилось би прямо зараз: `X архівів у Y групах, Z MB`.
- **Статус останнього / наступного запуску** -- коли остання щоденна обробка пройшла і коли пройде наступна.
- **Запустити очистку зараз** -- кнопка з підтвердженням, поважає ті самі правила пропуску, що й щоденна обробка.

### Як вирішується, що видаляти

Прийнятність обчислюється **per design**, а не per archive. "Дизайн" -- це група архівів, що ділять один `COALESCE(source_content_hash, content_hash)`.

- **Cutoff -- це найновіший друк по всій групі.** Якщо ви передрукували старий дизайн 3 дні тому, кожна архівна копія цього дизайну -- навіть рядки з місяців тому -- лишається. Намір -- "старі дизайни, від яких ви відмовились", а не "старі фізичні файли".
- **Правила пропуску -- якщо хоч одне з цих спрацьовує, *вся група* пропускається:**
    ...
    - Початковий рядок `library_files` все ще існує зі збіжним хешем. Бібліотека -- це ваше свідоме джерело правди, нема сенсу витирати архівну копію, коли вона все ще на джерельному шляху.
...
- Щоденна обробка опівночі за серверним часом.
- Тогл звіряється на кожному тіку, тож вмикання/вимикання діє без рестарту.
