...
Сторінка Архівів виставляє фільтр "duplicates", який групує рядки за цим ефективним хешем. Бейдж "оригінальний друк", лінк `original_archive_id`, список дублікатів у detail-ендпоінті, і лічильники `print_count` для файлів бібліотеки -- усі прив'язуються на `source_content_hash`, з `content_hash` лише як defence-fallback для legacy-NULL рядків. Два наслідки, які варто знати:

- Непатчений запуск на принтері A + запуск з вимкненим mesh-mode на принтері B того самого library-файлу тепер групуються в одному бейджі.
- Два архіви, які ділять один on-disk 3MF (cross-printer file dedup, нижче) лишаються окремими рядками з окремими `printer_id` -- дедуп тут суто про storage-layout.

### Cross-printer file-on-disk дедуп *(0.4.2)*

Коли BamDude архівує друк, чий `content_hash` збігається з наявним архівом на **будь-якому** принтері (раніше -- лише на тому самому), новий рядок архіву ре-юзає on-disk шлях замість писати ще одну копію. Ефект: друк одного файлу на N принтерах зберігає одну копію на диску + N рядків архіву. `delete_archive` ref-каунтує спільні `file_path` і прибирає байти лише коли видаляється останній посилаючий рядок.

Дедуп на **строгий збіг `content_hash`**, не на chain-coalesce `effective_hash` -- різні патчені варіації одного джерела ділять `effective_hash` для групування, але живуть окремими файлами на диску; aliasing між ними наводив би `content_hash` одного рядка на чужі байти і ламав би пізніші `ZipFile`-ріди. Той же дедуп тепер працює і в `attach_3mf_to_archive` (background download-retry) -- fallback-архів, чий 3MF приходить пізніше, підхопить існуючу копію, якщо інший принтер уже її зберіг.
