...
На сторінці **Черги** кожна картка черги принтера є drop-target — і кожна картка на сторінці **Принтери** теж. Кидайте **скільки завгодно файлів**: кожен завантажується в корінь бібліотеки, а далі проходить модалку Add-to-Queue по черзі, з лічильником `2 / 5` біля заголовка. Модалка припнута до цього принтера — без тогла specific/auto, і сам принтер показано відміченим, зняти вибір не можна, бо drop-target *і є* вибір. Статус принтера на картці (idle / printing / paused / error) **не** перевіряється: додавати в чергу можна завжди, незалежно від того, що принтер зараз робить.
...
Кнопка **Поставити в чергу** віддає виділення тій самій по-файловій модалці планування, що й дроп, із тим самим лічильником `2 / 5`. Сам пікер про друк не питає нічого: плити, мапінг AMS, кількість і розклад лишаються по файлу.
...
Далі відкривається звичайна модалка планування — **по одній на елемент**, з усіма обраними принтерами, відміченими й незнімними. Філамент мапиться per printer усередині цієї однієї модалки, тому три елементи на чотири принтери — це три діалоги, а не дванадцять.
