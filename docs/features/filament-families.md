...
A spool configured with **one of your own cloud presets** sends that preset, not the system one it was based on — with its own temperatures. A slicer preset belongs to a printer model, so on another model BamDude sends your variant of the same preset made for that printer and nozzle: the one with the same name apart from the "@printer" part, which is how Bambu Studio names a filament you create for several printers. When no such variant exists — or more than one would fit — the printer gets the system preset for its model instead (for a filament you created, the family alone, with your preset's temperatures). Which printer a preset is for is read from the preset it was based on, or from its name.
