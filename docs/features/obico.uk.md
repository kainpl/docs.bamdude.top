...
- Прапор **Детекція активна** (зелений / червоний) — зелений лише коли Obico ввімкнено і задано ML URL. Сам фоновий цикл стартує разом із BamDude і просто спить, поки фіча вимкнена; раніше прапор показував саме цикл і казав «Так» із вимкненим перемикачем.
...
### Бейдж AI на картках принтерів

Щоб бачити, як детекція веде друк, більше не треба сидіти на цій сторінці. Кожна картка на сторінці **Printers** несе бейдж **AI** поряд з іншими бейджами здоров'я — сірий, поки нічого не спостерігається, далі зелений / бурштиновий / червоний у міру класифікації друку. Тултип несе поточну оцінку; клік повертає сюди, до повного статусу й історії.

Принтер, якого детекція **не** спостерігає, не показує бейджа взагалі, а не такий, що натякає на покриття.

Бейдж ніколи не каже, що друк спостерігається, коли це не так. Поки друк під наглядом чекає на перший результат, він пише **AI запускається** (сірий). Коли остання перевірка не дала результату — ML-сервер відхилив токен, був недоступний, камера не дала кадру або порожній External URL, — він пише **AI не перевіряє** (бурштиновий), а тултип називає причину для цього принтера (причину бачать користувачі з доступом до налаштувань; решта бачить, що друк не перевіряється). Без результату оцінка не показується. Список активних друків на цій сторінці показує те саме, а **Test** спершу зберігає форму, тож зелений результат описує конфігурацію, з якою детекція справді працює.

!!! info "Бейджу не потрібне право на налаштування"
    Його бачить кожен, хто бачить принтери. Він свідомо не несе жодної
    конфігурації — адреса ML-сервера й історія детекції лишаються за
    `settings:read`, там, де були.
