...
## :material-content-duplicate: Дублювання проєкту

**Duplicate** на проєкті копіює його *налаштування* в новий проєкт і лишає позаду його *історію* — сенс саме в тому, що ви збираєтесь зібрати те саме ще раз: для другої машини чи для іншого замовника.

Копіюється: план друку (кожен рядок зі своїми копіями й порядком), звʼязки з файлами й теками, BOM, обидві цілі, опис, нотатки, вкладення, теги, дедлайн, пріоритет, бюджет, колір, обкладинка й URL. Не копіюється: архіви — вони належать друкам, які справді відбулись.

Дві речі зроблені навмисно, а не випадково:

- **Копія завжди `active`**, яким би не був оригінал. Дублікат завершеного чи заархівованого проєкту — це нова робота, яка ось-ось почнеться, і саме заради цього його й дублюють.
- **Дублікат шаблону — теж шаблон.** Прапорець описує, чим проєкт **є**, а не що з ним сталося.

Дайте копії ім'я або лишіть BamDude обрати його; вона стає **сусідом** джерела, під тим самим батьком. Підпроєкти йдуть слідом, лише якщо попросити (`include_children`), і цикл у батьках не зациклить копіювання.
...
Архів друку записує, скільки об'єктів було на плиті. Тепер він записує ще й скільки з них вийшли **непридатними** — повело, недолив, зламалась при знятті. Познач кількість браку на архіві — і лічильник деталей проєкту буде вже за вирахуванням.

Це важливо, бо проєкт, якому треба сорок придатних деталей, не завершений тоді, коли сорок зійшло з плит. Без цього лічильник дійде до цілі, а робота — ні. На картці проєкту брак показано поруч із загальним числом, тож серія, яка тихо жене шлюб, помітна ще до кінця.

!!! tip "Ціль у нуль означає «так не міряємо»"
    Якщо працюєш строго по кількості деталей — постав **Target Plates** у `0`, і смуга прогресу по плитах зникне, замість висіти з числом, за яким ніхто не стежить. У зворотний бік працює так само.
