...
- **Search-box** — матчить **ім'я котушки, зібране твоїм шаблоном відображення**, а не лише сирі колонки за ним, тож знайти можна все, що показано у списку: із шаблоном `{brand}/{material}` запит `LU/PET` спрацює. Бренд, матеріал, колір, підтип, нотатка і slicer-preset матчаться завжди, як і **id** та **номер лота** котушки — ці два незалежно від шаблону, бо це числа, написані на самій котушці. Кожен токен має щось зматчити, тож `SUN Bl` знаходить SUNLU Black. Натисни `/` будь-де на сторінці, щоб сфокусуватися.
...
### Редагування котушки, створеної через Quick Add, імпорт CSV чи RFID

Ці три шляхи створюють котушку з матеріалом і майже нічим більше — без slicer-пресета, без бренду, без підтипу. **Тепер редагування й копіювання питають рівно те, що питає сервер: матеріал.**

Раніше відкриття такої котушки в Edit вимагало пресет, бренд *і* підтип, перш ніж узагалі щось зберегти, — тож змінити її полицю зберігання, ціну чи нотатку було просто неможливо, а перемикач Quick Add, який знімає ці вимоги, існує лише при створенні. Copy Spool упирався в ту саму стіну, і обійти її не було як.

Пресет, бренд і підтип лишаються видимими й редагованими, а маленькі позначки `*` з'являються лише там, де поле справді обов'язкове.

!!! info "Списки брендів і матеріалів більше не ховають реальні продукти"
    Вони фільтрувалися до тих пар бренд/матеріал, які знає колірний каталог. Elegoo
    занесений у каталог лише для PLA — тож **Elegoo ASA**, реальний філамент, який
    можна купити, виглядав як щось, чого не можна ввести.

    Тепер обидва списки завжди пропонують усе: відомі пари першими під
...
## :material-checkbox-multiple-marked: Масові дії

Кожен рядок на вкладці Філаменти має чекбокс. Познач один — і над списком з'явиться панель із діями **Редагувати · Друк етикеток · Скинути витрати · Архівувати** (або **Відновити** на вкладці Архів) **· Видалити**.

Чотири способи виділення:

| Контрол | Що позначає |
|---|---|
| Чекбокс рядка | Цю котушку |
| Чекбокс групи | Усі котушки групи (показує риску, коли позначено лише частину) |
| Чекбокс у шапці | Усі котушки **на поточній сторінці** |
| **Вибрати все N за фільтром** | Усі котушки, що їх повертає поточний фільтр, включно з рядками на інших сторінках |

Останній пункт — свідомо окрема явна дія, а не поведінка за замовчуванням. Редагувати «все, що показує фільтр» зручно; *видаляти* це не має ставатися через галочку, якої ти не бачив. З тієї ж причини виділення скидається саме, щойно ти міняєш фільтр, пошук, вкладку чи групування — панель із написом «вибрано 12» над списком, де тих котушок уже нема, це рівно той сценарій, у якому масове видалення прилітає не туди.

Архівування, відновлення та видалення спершу перепитують.

Усе це працює і в **режимі Spoolman**, не лише з вбудованим інвентарем. Оскільки Spoolman — окремий сервіс, який може бути тимчасово недоступним, окремі рядки можуть не пройти: тоді ти побачиш чесне **«7 виконано, 2 не вдалося»** замість повідомлення про успіх, а виділення збережеться, щоб можна було повторити для решти.

!!! tip "Діалог редагування показує, що саме зміниться"
    Діалог масового редагування перелічує конкретні котушки з виділення. Він лише для читання — вибір робиться на сторінці — але варто його прочитати, перш ніж застосовувати поле до сорока котушок.
...
- **Auto-tracking нових Bambu-котушок** — коли AMS RFID не матчить жодного існуючого tray UUID, BamDude спочатку шукає **untagged**-котушку зі збіжним матеріалом + кольором + брендом (`Bambu` / `Bambu Lab` / unspecified) і прив'язує RFID до неї. Тож Quick-Add stock-запис, що ти залогував наперед, реюзається (твоя вага, нотатки, cost-data зберігаються) замість того, щоб видавати дублікат. Якщо збігу немає — створюється свіжий inventory-рядок з AMS-data, намальований так само, як котушка того ж кольору, додана вручну: назва кольору, додаткові кольори й ефект беруться з каталогу кольорів, а де каталог ефекту не має — його дає підтип, якщо той називає оздоблення (*Wood*, *Silk*, *Sparkle*, *Gradient*…; *Basic* чи *CF* не називають жодного).
...
- **Зайнято під замовлення** — скільки **відомих** грамів активні замовлення ще мають забрати з полиці: ще не надрукований план плюс те, що вже чекає в чергах, пораховано так само, як на сторінках замовлень, і розкладено по SKU, які можуть це покрити. Якщо в активному замовленні канал слайсера не має достовірної ваги, панель попереджає: **Зайнято** й **Замовити до** враховують лише відому частину; BamDude не вигадує грами й не приписує таку невідомість конкретному SKU.
...
### Заміна котушки на призначеному слоті

Знімати призначення заздалегідь не треба. Наведи на призначений слот і обери **Замінити котушку** (поруч з **Unassign**): діалог показує, що зараз призначено, і пропонує лише вільні котушки — поточна і будь-яка призначена в інший слот приховані, тож випадково «витягти» котушку з іншого принтера неможливо — і одне підтвердження міняє призначення. Працює однаково на слотах AMS, AMS HT і зовнішніх, із вбудованим інвентарем і зі Spoolman.

В обліку нічого не змінюється: заміна проходить рівно тим самим шляхом, що й зняття з наступним призначенням. Посеред друку з'являється та сама підказка — на паузі або після паузи — з питанням, чи це фізична заміна котушки (витрата філаменту ділиться по шару паузи), чи виправлення хибного зв'язку (весь друк іде на нову котушку).
...
Inventory-сторінка кольорує кожну котушку за залишком %, з налаштовним **low-stock threshold** (Settings → Inventory). Коли котушка падає нижче — спрацьовує нотіфікація `filament_low` (підпишись на тих провайдерах, де це треба). Котушку без прив'язки до інвентарю судять за лічильником залишку самого принтера (лише RFID-котушки Bambu), слот із прив'язкою до Spoolman — за цифрою Spoolman, проти того самого порога.
...
### Runout закриває котушку рівно в нуль (0.5.5)

Runout — єдиний момент, коли реальність повідомляє точну цифру: на котушці **0 г**, хоч би що казали книги. BamDude слухає **власні runout-коди принтера** — AMS-слот чекає філамент, AMS автоматично перемкнувся на резервний слот, або закінчилась зовнішня котушка — і перетворює кожен на точний облік. Застрягання й заплутування *ніколи* не сприймаються як runout (рахуються лише точні коди прошивки), а котушка, чий слот не визначено однозначно, ніколи не чіпається.

Що відбувається, по порядку:

1. Runout журналиться на точному шарі, з котушкою, що фідила, **замороженою на той момент**.
2. Приходить нотифікація **Закінчився філамент** (увімкни її на своєму провайдері): або *«чекає філамент — признач заміну в BamDude, щоб решта друку лягла на правильну котушку»*, або інформаційна *«перемкнувся на резервний слот»*.
3. Заправляєш нову котушку. RFID-котушку BamDude побачить сам по тегу; для безтегових (зовнішні холдери включно) **сигналом є призначення заміни на слот** — зроби це будь-коли до кінця друку.
4. На завершенні друк розрізається **по runout-шару** (пошарова G-code-точність): кожна котушка отримує лише свій сегмент. Потім витрачена котушка отримує один фінальний рядок **«Закриття по runout»** на все, чого книги не бачили — її історія сходиться рівно з вагою етикетки, залишок стає точним нулем.

**Приклад.** Книги котушки A кажуть 2 кг залишку; друк на 600 г обривається runout'ом у точці, де реально з'їдено 250 г; заправляєш котушку B і додруковуєш.

| Рядок | Котушка | Грами |
|---|---|---|
| Сегмент друку до runout-шару | A | 250 г |
| Сегмент після | B | 350 г |
| Закриття по runout (помаранчевий рядок історії) | A | **+1750 г** |

A закінчує рівно з 0 г залишку (250 + 1750 = її 2 кг етикетки); B несе лише чесні 350 г. Архів друку так само показує 600 г і вартість за 600 — закриття є визнанням життєвого дрейфу котушки, а не витратою цього друку. Якщо B *не* призначиш — усі 600 г лишаться на A, закриття зменшиться до 1400 г, A все одно закінчить у нулі, а B лишиться необлікованою — саме тому нотифікація і підштовхує призначити.

Два runout'и одного слота за один довгий друк (два коротких хвости підряд) обробляються як окремі епізоди — кожен зі своєю межею і закриттям. У Spoolman-режимі все працює ідентично. Налаштовується в **Settings → Filament → Точність списання**: тумблер закриття, опційна доплата за продувку на автоперемиканні (аварійного пуржа нема в оцінці слайсера), двобічний AMS-синк нижче, і скільки тримати по-принтовий журнал подій для розбору (72 год за замовчуванням).

### Двобічний AMS-синк ваги для тегованих котушок (0.5.5)

Для котушок із валідним Bambu RFID-тегом показання AMS на простої тепер коригують книги і **вниз**, не лише вгору — оцінка залишку прошивки це те саме число, яке Bambu Studio дзеркалить у свій filament manager. Корекція вниз навмисно обережна: те саме значення має повторитися у двох звітах з інтервалом щонайменше хвилину, тож збитий звіт після реконекту ніколи не перепише котушку. Безтегових котушок цей шлях не торкається.

### Живе «Філаменту вже» (0.5.5)

Поки друк іде, розгорнута картка принтера показує, скільки філаменту він уже з'їв — пошарова G-code-точність, per-slot на мультиколірних, оновлення двічі на хвилину — і каже **«розділено між котушками»**, щойно runout реально поділив друк між рулонами. Тільки відображення: у книги, як і раніше, пишеться один раз, на завершенні.

#### Mid-print spool-change семантика

- **Після runout'а** — атрибуція ріжеться по runout-шару, як описано вище: межею володіє журнал.
- **Без runout'а** (ре-assign слота посеред друку для виправлення хибного лінка) — виграє live-assignment, і весь друк білиться новопризначеній котушці. Це семантика виправлення: BamDude вважає, що лінк був хибним із самого початку, а не що ти міняв рулони.
...
### Режим History — усі списання одразу {#history}

Третій режим перегляду інвентаря, **History**, між Cards і Forecast, показує записи списань усієї ферми, а не однієї котушки: що друкувалось, з якої котушки, на якому принтері, скільки грамів, скільки коштувало і чим закінчилось — найновіше зверху.

Вкладка самої котушки відповідає на «куди пішла ця котушка». Цей режим — на «куди пішов філамент», а саме з цього питання зазвичай і починають. Записи ті самі, різниця лише в точці входу.

Усе в цьому списку рахує **сервер** — сторінка, порядок, фільтри, пошук і підсумки. У ферми з роком друку тут шестизначна кількість записів, і браузер їх не завантажує.

- **Пошук** — те саме поле пошуку сторінки. Той самий бокс, те саме місце; просто шукає воно тепер події — за назвою друку, котушкою і принтером одночасно.
- **Фільтри** — принтер (разом із *Без принтера*, для записів, що не числяться за жодною машиною), матеріал, бренд, статус і діапазон дат. Плюс стан самої котушки: **Всі / Активні / Архівовані** і **Всі / В принтері / На полиці**. Останні дві групи мають варіант *Всі*, якого немає на таблиці котушок, і він дефолтний: вихід котушки в архів чи зняття з принтера не скасовує того, що вона надрукувала, тож нічого тут не ховається, доки не попросиш.
- **Сортування** — за будь-якою колонкою: дата, котушка, друк, принтер, грами, відсотки, вартість, статус.
- **Підсумки** — грами (і гроші) поруч із фільтрами рахуються по **всьому фільтру**, а не по сторінці на екрані. «Скільки коштував серпень» — це один діапазон дат.

Котушки, які ти відтоді заархівував чи видалив, зберігають свої рядки — позначені, а не сховані: грами, які вони несуть, усе одно були надруковані, а викидання їх розсинхронізувало б ці підсумки з [архівами](archiving.uk.md). Імена котушок ідуть за твоїм шаблоном відображення (Settings → Inventory → Spool display name template), а архівний принтер читається як *Printer 5 (Archived)* — так само, як усюди. Клік на котушці відкриває її.

Режим **прихований у Spoolman-режимі**, де цей журнал веде Spoolman.

### Видалення записів витрати

Кожен рядок має **×** при наведенні, щоб видалити саме цей запис — і у вкладці **Історія** самої котушки, і в загальнофермовому режимі History; кнопка **Clear** у вкладці котушки робить те саме для всього її списку. Видалення запису трактує ту витрату так, ніби її не було: його вага **повертається до котушки** (`weight_used` зменшується, тож залишок росте), і та сама величина віднімається від облікованого пластику привʼязаного друку, тож сторінка [Stats](stats.uk.md) лишається синхронною з інвенторі. Для мультиколірного друку віднімання — поелементне: видалення запису одного кольору повертає лише його частку й лишає решту друку без змін. Зручно, щоб «не рахувати» помилковий чи тестовий друк проти рулону.
