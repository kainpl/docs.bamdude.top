...
| `status` | Локально `submitted` / `failed`; освіжається зі стану GitHub-issue до `open`, `closed` (виправлено) чи `not_planned` щоразу, коли відкривається список **«Мої репорти»**. |
