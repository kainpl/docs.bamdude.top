...
3. На завершенні кожна котушка звітується індивідуально — multi-material-друк оновлює кожну прив'язану котушку окремо. Лоток кожного філаменту — той, з яким друк було відправлено; для друку, який відправив не BamDude і чий мапінг він не бачив (A1 його не публікує), одно-філаментний друк списується з лотка, який назвав сам принтер. Лише коли лотка не назвало ніщо, списання йде за позицією лотка — це здогадка: вона пишеться в лог як попередження, і тоді архів лишає колір і матеріал з нарізаного файлу, а не бере їх із котушки, обраної за позицією.
...
**Архів дізнається, що списано.** Кожне списання, що пройшло, оцінюється за власною котушкою — ціна котушки, якщо задана, інакше ціна її філаменту, поділена на чисту вагу філаменту, — тож багатоматеріальний друк рахує кожен слот за його ціною, а грами, які жодна котушка не оцінила, покриваються типовою ціною філаменту ферми. Вага філаменту в архіві йде за тим самим правилом, що й у вбудованому інвентарі: друк, що впав, або друк без 3MF записує реально списані грами; завершений друк лишає оцінку слайсера. Котушка без ціни не чіпає записаної вартості. **Перерахувати вартість** на сторінці статистики лишає так оцінену вартість, а не переоцінює її за типовою ціною, і каже, скільки лишила.
...
| **Color** | Назва кольору + swatch — резолвиться через color-каталог BamDude (єдине джерело правди). У Spoolman немає поля назви кольору, тож котушка, чию назву BamDude не зберігав, показує назву з каталогу для свого swatch, а не свій підтип (`Silk+` — не колір); пошук знаходить котушку за назвою кольору, яку видно. |
