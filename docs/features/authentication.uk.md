...
!!! info "Auth завжди увімкнений"
    Перемикача "вимкнути auth" немає. Кожен ендпоінт вимагає валідної сесії або API-ключа. API-ключі (`X-API-Key` або `Authorization: Bearer bd_...`) обходять JWT-перевірку, але все одно проходять ту саму перевірку прав.
...
Опційний SMTP-шар, що дає passwordless onboarding, скидання пароля адміном і per-user сповіщення про друк. Тогглиться незалежно від базового auth.

!!! info "Самостійне скидання пароля цього тумблера **не потребує**"
    Відновленню потрібен поштовий сервер — це єдине, що воно перевіряє. Досить налаштувати SMTP — див. [Самостійне скидання пароля](#самостійне-скидання-пароля). До 0.5.6 відновлення було привʼязане до цього перемикача, тож адмін, який налаштував SMTP, надіслав тестовий лист і не здогадався про другий вимикач, отримував *«Advanced authentication is not enabled»* від посилання, яке login-сторінка все одно показувала.
...
- **Password reset link** — самостійне відновлення; несе одноразове посилання, ніколи не пароль
- **Password reset** — скидання, ініційоване адміном; несе сам новий пароль
...
### Самостійне скидання пароля

**Доступне, коли налаштований SMTP і увімкнений локальний вхід.** Коли ні — замість посилання login-сторінка показує простий рядок *«зверніться до адміністратора»*, а не форму, яка збере адресу й мовчки нічого не зробить; адмін відновлює акаунт [з консолі сервера](#скидання-пароля-з-консолі).

1. Користувач клікає **Forgot your password?** на login-сторінці й вводить свою **email-адресу**.
2. Ендпоінт відповідає однаково в будь-якому разі (анти-енумерація) і надсилає щось лише якщо адреса належить активному локальному акаунту.
3. Лист несе **одноразове посилання**, дійсне **1 годину**. Новий запит гасить попереднє посилання.
4. Посилання відкриває форму *«Встановити новий пароль»*. Пароль підлягає [паролевій політиці](#парольна-політика).
5. Після успіху користувач повертається до форми входу й заходить з новим паролем. Усі сесії, що були на старому паролі, розлогінюються.

!!! success "Акаунт не змінюється, доки посилання не використали"
    Сам запит не робить нічого. До 0.5.6 запит генерував новий пароль і надсилав його — тобто знання чужої адреси дозволяло ротувати пароль і вибити людину з робочої сесії, а вона навіть не бачила листа, який це зробив. Тепер пароль змінюється лише коли хтось довів, що прочитав листа.

    Токен посилання зберігається лише як хеш, тож копія бази — це не набір робочих посилань для скидання.

Скидання, ініційоване адміном, — **інший** флоу: **Settings -> Users -> Reset password** надсилає користувачу щойно згенерований пароль, і от воно якраз вимагає тумблер Advanced Auth.
...
### Оголошення провайдера через змінні середовища

Для інсталяцій, якими керує compose-файл, Helm чи GitOps, де «один раз клікнути в Settings» не є частиною деплою. Постав ці змінні — і провайдер створюється на старті та **перезастосовується при кожному рестарті**:

```bash
# Обов'язкові — усі чотири, інакше нічого не застосовується
...
Опційні змінні мають ті самі типові значення, що й в UI:
...
| `BAMDUDE_OIDC_ICON_URL` | *(порожньо)* |
| `BAMDUDE_OIDC_AUTOLOGIN` | `false` |
| `BAMDUDE_OIDC_DEFAULT_GROUP` | *(не задано — береться типове значення UI)* |

Булеві приймають `true/1/yes/on` і `false/0/no/off`.

!!! warning "Провайдер з env — read-only в Settings"
    Оскільки змінні перезастосовуються при кожному завантаженні, провайдер показується як read-only, і застосунок відмовляється змінювати його там. Редагування було б прийняте й тихо скасоване наступним рестартом, а це гірше, ніж пряме «ні».

    **Прибрати змінні — це вимкнути провайдер, а не видалити його.** Акаунти, вже прив'язані до нього, зберігають прив'язку й повертають її, коли змінні повернуться.

!!! info "Усі режими відмови тут тихі — тому про них сказано голосно"
    - Обов'язкова змінна, лишена **порожньою**, вважається незаданою, а не порожнім секретом.
    - Значення **обрізаються** — секрет, змонтований із файлу чи записаний як block scalar у Kubernetes, не протягне за собою кінцевий перенос рядка, який упав би тільки на першій спробі входу.
    - **Нерозпізнане** true/false відхиляється з рядком у лозі, а не вгадується, і вся конфігурація пропускається: те, що вже працювало, працює далі.
    - `BAMDUDE_OIDC_DEFAULT_GROUP` — це **назва** групи, а не ID: ID різні на кожній інсталяції. Назва, якій не відповідає жодна група, відхиляє конфігурацію, а не падає тихо на Viewers.
    - Погана конфігурація **ніколи не заважає застосунку стартувати**.

Усе вищезгадане також описане в `.env.example`.
...
| `POST /auth/forgot-password/confirm` | — | 10 / 15 хв |
...
### Як правила показуються на формі

Кожне поле, де задають **новий** пароль — first-boot setup, створення користувача, редагування, зміна власного пароля і сторінка за посиланням для скидання — показує під собою всі чотири вимоги й зеленими галочками відмічає виконані під час набору. Вони зʼявляються щойно ви клікнули в поле, тож видно, чого від вас хочуть, ще до відмови, а не після неї.

Біля кожного поля пароля праворуч є «око», щоб побачити введене.

!!! note "Виправлено в 0.5.6"
    До цього вимог на екрані не було взагалі: форми блокували кнопку відправки за правилами, а повідомлення з назвою порушеного правила було написане так, щоб спрацювати при натисканні кнопки, яку це саме правило щойно вимкнуло. Задовгий пароль давав сіру кнопку й жодного пояснення. Дві форми ще й перевіряли **6** символів, тоді як API давно вимагає 8 і суміш регістрів з цифрою — тобто приймали паролі, які сервер потім відхиляв. А сам API не застосовував поріг у 8 символів, коли адмін створював чи редагував користувача, — лише на setup і зміні пароля.
...
Дві команди для двох різних ситуацій. Обидві запускаються з shell-у на хості, проти тієї ж бази, яку використовує сервер.
...
!!! warning "Запускайте при зупиненому сервері"
    ...
    ```bash
    docker compose stop bamdude     # АБО: systemctl stop bamdude
    ```

### Скидання пароля з консолі

Це для випадку, коли акаунт існує, але в нього ніхто не може зайти — забутий пароль на інсталяції **без поштового сервера**, де самостійного відновлення немає взагалі. У того, кому належить машина, є на ній shell — і це та сама людина, якій пішов би лист про скидання.

```bash
# Хто взагалі є?
python -m backend.app.cli list_users

# Запитати новий пароль (двічі, без відлуння на екрані).
python -m backend.app.cli reset_password --username admin

# Або згенерувати й показати один раз.
python -m backend.app.cli reset_password --username admin --generate

# Другий фактор загублений разом із паролем? Зняти і його.
python -m backend.app.cli reset_password --username admin --clear-2fa
```

- Новий пароль проходить **ті самі правила**, що й веб-форма, тож з консолі не можна задати такий, який акаунт уже ніколи не зміг би задати через інтерфейс.
- Усі сесії, що були на старому паролі, **розлогінюються**.
- `--clear-2fa` знімає TOTP, email-OTP і резервні коди. Без цього адмін, який загубив і пароль, і автентифікатор, усе одно не зайде.
- LDAP-акаунтам відмовляє з поясненням: їхні паролі живуть на сервері каталогу, тож задати щось тут — не змінити нічого.
- Деактивований акаунт про це повідомить: пароль буде задано, але акаунт усе одно треба спершу увімкнути.

### Повторно пройти first-boot setup

Це для випадку, коли адмінського акаунта **не лишилось узагалі** — останній видалили.

```bash
python -m backend.app.cli reset_admin
```

`reset_admin` чистить прапор setup-complete, тож наступний старт знов потрапляє у форму **first-boot setup**, де ви створюєте нового адміна з нуля. **Усі ваші існуючі дані (принтери, архіви, черга, користувачі, бібліотека) збережуться.**

!!! note "Поки хоч один адмін існує — команда відмовляє"
    Це навмисно: інакше вона була б способом обійти логін, який ви просто забули. Якщо адмінський акаунт на місці, а пароль до нього втрачено — використовуйте `reset_password` вище.
...
### Забув пароль (SMTP нема)

Без налаштованого поштового сервера самостійного відновлення не існує, тож login-сторінка показує *«зверніться до адміністратора»* замість посилання. Адмін задає новий пароль зі **Settings -> Users -> Edit** або — якщо залочений саме адмін — [з консолі сервера](#скидання-пароля-з-консолі).

### На login-сторінці немає «Forgot your password?»

Воно показується лише коли лист справді може піти: **налаштований SMTP** і **увімкнений локальний вхід**. Спершу перевір **Settings -> Email** і натисни **Test email** — посилання зʼявиться щойно сервер зможе слати пошту. Від тумблера Advanced Auth воно **не** залежить.

### Посилання для скидання каже, що воно недійсне або застаріло

Посилання живе **1 годину** і працює **один раз**. Запит нового ще й гасить попереднє, тож старіший лист у скриньці повідомить саме це. Попроси нове. Якщо будь-яке посилання відмовляє одразу — перевір, що **External URL** у **Settings -> Email** це та адреса, за якою користувачі реально заходять: посилання будується з неї.
