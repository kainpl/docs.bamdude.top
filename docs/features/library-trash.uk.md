...
!!! warning "Відновлення файла бібліотеки питає, перш ніж створити дубль"
    Усі інші шляхи тепер відмовляються зберігати той самий файл двічі, тож дістати його з кошика — єдиний спосіб, що лишився, отримати два однакові файли. Відновлення спершу перевіряє й озивається **лише якщо файл справді побайтово збігається з тим, що у вас уже є** — діалог називає наявний файл, а рішення за вами.

    Відновлення кількох одразу питає **один раз**, перелічуючи, які саме з них дублі, і пропонує відновити лише решту. Файл, до якого нічого не збігається, відновлюється як раніше, без питань.

    Воно питає, а не відмовляє. Дубль може бути навмисним — два профілі MakerWorld можуть дати побайтово однакові 3MF — і саме та людина, яка поклала файл у кошик, зараз його звідти й дістає.

!!! danger "Кошик архівів до 0.5.4 насправді нічого не видаляв"
    «Очистити кошик» повідомляв *видалено 0* і не прибирав нічого взагалі; **Видалити назавжди** на одному архіві віддавало помилку; а сміттяр ретенції зносив рядки з бази, лишаючи на диску кожен 3MF, прев'ю, таймлапс і фото. Полагоджено у 0.5.4 — але файли, що натекли до того, лишаються на диску, і BamDude не піде видаляти їх без запиту. `scripts/prune_orphan_archive_files.py` показує, на що більше немає посилань, і видаляє лише з `--apply`.
