...
2. **Ціль камери** обчислюється з філаментів, які використовує друк: BamDude шукає тип кожного в [мапі цілей камери](#мапа-цілей-камери-на-філамент) і бере **найгарячішу** — тож змішане завдання PA + PLA soak-ається під PA. Коли виходить `0` (PLA, PETG, TPU, PVA), прогрівати нема для чого, і **стадія пропускається повністю** — друк стартує одразу; на принтерах із заслінкою повітропроводу її повертають в охолодження. Ціль камери рівно `0`, введена для конкретного друку, або прогрів, примусово увімкнений для нього, і далі виконують фазу столу й soak.
3. **Ціль столу** читається з метаданих самого файлу друку. Якщо файл її не несе, а друку потрібна камера, стіл гріється до 90 °C — на цих принтерах камеру гріє саме стіл.
...
Ціль камери для друку виводиться з філаментів, які цей друк використовує. Вбудована мапа несе такі дефолти (°C), повністю редаговані під **Settings → Printing → Preheat & Heat Soak**:
...
- **`0` означає «нема для чого прогрівати»** — commodity-філаменти (PLA, PETG, TPU, PVA) виводять `0`, тож друк лише з PLA повністю пропускає прогрів і стартує одразу.
- **Найвища ціль серед філаментів, які використовує друк, перемагає.** Друк, що бере PA зі слота 1 і PLA зі слота 2, soak-ається під 50 °C PA: вимога інженерного філаменту є зв'язуючою.
- **Котушка, яку друк не використовує, не враховується.** Які слоти бере друк, визначає його мапінг філаментів, тож котушка ASA у слоті 3 не змушує друк лише з PLA чекати камеру на 45 °C. Зовнішня котушка, з якої друк подається, враховується нарівні зі слотами. Друк без мапінгу філаментів, як і раніше, враховує всі завантажені слоти AMS.
- **Наповнений чи спінений тип без власного рядка бере рядок свого базового матеріалу.** Слот з ASA-GF чи ASA Aero отримує 45 °C від ASA, ABS-GF — значення ABS. Тип, що має власний рядок, як і раніше перемагає: PETG-CF лишає свої 40 °C, а не 0 від PETG. Лише тип, чиєї бази теж немає в мапі, падає на **Інше / не в мапі**.
...
Коли не виставлено **Off**, опційне поле **Chamber target override** дозволяє ввести явну температуру камери (°C) для цього друку. Лиши порожнім, щоб використати дефолт, виведений з філаменту, — тоді друк, філаменти якого не потребують камери, повністю пропускає прогрів; явний `0` означає «стіл і soak, але без фази камери», навіть якщо філамент інакше хотів би її. Підказка під полем про це й каже.
...
- **Зовнішня котушка враховується лише тоді, коли її називає мапінг філаментів друку.** Друк без мапінгу виводить ціль лише з AMS, тож такий друк із зовнішньої котушки дає `0` і пропускає прогрів — застосуй per-print chamber override, якщо там потрібен soak камери.
