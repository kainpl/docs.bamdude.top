...
Перетягніть нарізаний `.gcode.3mf` файл на картку принтера або клікніть зелену **Print**. Перетягнутий файл заливається в бібліотеку, і відкривається діалог Schedule, закріплений за цим принтером, — задача стає в його чергу; кнопка **Print** заливає файл і відкриває print-modal для прямого друку.

Файл приймається, хоч би що робив принтер — друкує, на паузі чи офлайн, — бо він лише стає в чергу: overlay пише **Drop to print**, коли задача почнеться одразу, і **Drop to queue**, коли чекатиме друку, плити чи повернення принтера. Для перетягування потрібні дозволи **Upload** і **Add to queue**; без одного з них overlay називає, якого бракує. Що відбувається після dispatch — у [Print Queue](print-queue.md).
...
**Вимкнений** принтер теж приймає відповідь, без перевірки стану: з Auto Power Off «плита ще зайнята, принтер вимкнено» — звичайний кінець друку, а очищення нічого не надсилає на принтер. Картка лишає кнопку **Clear plate** на такому принтері (і масове *Clear plate* теж працює); далі черга вмикає принтер під наступну задачу замість того, щоб його пропускати. Увімкнений принтер, що друкує, як і раніше отримує відмову.
...
`distance` — це **зазор між соплом і столом**, однаковий за змістом на кожній моделі: додатне значення збільшує зазор, від'ємне — зменшує. На CoreXY-принтері (X1, P1, H2, P2S, X2D) стіл відходить від сопла; на bed-slinger (A1, A1 Mini, A2L) голова піднімається над столом. Додатне значення — завжди безпечний напрямок.

!!! warning "Змінено після 0.6.1"
    Раніше `distance` жив за конвенцією *стрілок* Bambu Studio — від'ємне значення піднімало те, що рухається по Z, — а на bed-slinger це протилежне зазору: скрипт, що слав `+5` заради запасу на A1, опускав сопло. Стрілки картки принтера тепер ідуть через `POST /api/v1/printers/{id}/jog?axis=z`, де конвенція стрілок Studio збережена.
...
Ця ж вісь є і у вікні **Керування рухом** нижче — разом із X, Y та екструдером. Обидва йдуть через одну реалізацію, тож обробка напрямку й послідовність з ендстопами не можуть розійтись. На bed-slinger обидва підписують керування **Z** і говорять про голову, а не про стіл — там саме вона рухається по Z, і так це підписує Bambu Studio.
