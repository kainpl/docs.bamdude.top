...
Живі fan-badge у controls row — **Part Cooling**, **Auxiliary**, **Chamber** — показують real-time швидкості (0–100 %), що репортить принтер.

!!! info "Три стандартні бейджі — read-only"
    Їхні швидкості визначає slicer-профіль і прошивка принтера. BamDude їх показує,
    але не дає контролу.

### Вентилятори повітроводу (P2S / X2D) — керовані

Машини з **повітроводом** мають вентилятори, які ніколи не приходять як звичайне поле швидкості. Вони з'являються власними бейджами, **з контролом швидкості просто на бейджі**: `POST /printers/{id}/fan-speed?part_id=<n>&percent=<0-100>`, за правом `printers:control`.

Найпомітніший із них — **другий допоміжний вентилятор**: комплект-доповнення на **P2S**, заводський на **X2D**.

- **З'являється лише те, що встановлено.** Принтер перелічує реально наявні вузли, тож бейдж керується залізом, а не списком назв моделей. Id, якого принтер ніколи не повідомляв, відхиляється (**400**), а не надсилається: команду б прийняли, і вона б нічого не зробила.
- **Вентилятори називаються так, як їх називає твій принтер.** Той самий part id — це різні вентилятори на різних моделях: part 10 — *лівий* допоміжний на P2S і *правий* на X2D, а на X2D він ще й називається по-різному в режимах охолодження та нагріву. Назви беруться з визначень принтерів Bambu Studio, які BamDude і так поставляє, — тож P2S каже **Right Auxiliary Fan**, а не просто «Auxiliary».
- **Вентилятор, який поточний режим повітроводу тримає вимкненим, показується, але не пропонується як контрол** (**409**, якщо спробувати) — принтер приймає команду й ігнорує її.
