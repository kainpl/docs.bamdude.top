...
|-------|----------------------------|----------|
| **`file_manager`** (дефолт) | Аплоад зберігається в **бібліотеку [File Manager](file-manager.md)**, і більше нічого не відбувається — ні друку, ні черги. Оператор друкує його звідти, коли буде готовий. | Multi-user / multi-machine inbox, де кожен аплоад спершу переглядають — також правильний режим, якщо ви хочете просто **зберегти** файл без друку. |
...
!!! note "Чому слайсер пише «Finished», показуючи при цьому прогрес"
...
## :material-folder-arrow-down: Що робить режим `file_manager`

Аплоад зберігається просто в **бібліотеку [File Manager](file-manager.md)** як звичайний файл — той самий рядок, ті самі теки й теги, що й у файлів, завантажених через UI. Нічого не друкується і нічого не ставиться в чергу; файл просто лежить, доки хтось із ним щось не зробить.
...
!!! info "Окремої черги рецензування більше немає"
...
## :material-swap-vertical-variant: Використовувати слоти AMS зі слайсера {#save-ams-mapping}

**Вимкнено типово, на кожен віртуальний принтер окремо** — на картці VP поряд з іншими Queue-mode перемикачами.

Коли слайсер надсилає завдання через Queue-mode VP, BamDude зазвичай ігнорує номери слотів, які визначив слайсер, і підбирає лотки сам — за типом і кольором філаменту. Майже завжди це правильний дефолт, але **дві котушки того самого червоного PLA для файлу нерозрізненні**, тож підбір за типом і кольором не відрізнить їх. Хоч яку ти обрав у Bambu Studio, BamDude може зарядити іншу.

Увімкни це — і слоти, які визначив слайсер, використовуються рівно так, як надіслані.

!!! warning "Це компроміс — розберись до того, як вмикати"
    Для завдань із цього VP BamDude перестає обирати слоти, а разом з тим вимикаються:

    | Що втрачаєш | Що воно зазвичай робить |
    |---|---|
    | **Спершу найменша котушка** | Дотискає почату котушку, перш ніж відкривати свіжу |
    | **Гейт AMS-Backup** | Не дає «спершу найменшій» лишити друк без філаменту, коли AMS backup вимкнено |
    | **Залишок ваги з інвентаря** | Пропускає лоток, якого, за інвентарем, не вистачить на завдання |
    | **Маршрутизація Flow-Through-System** | Виводить FTS-слот на зовнішній тримач котушки |

    Коли налаштування **вимкнене**, усе це працює точно так, як і зараз. Мапінг, який слайсер лишив цілком невизначеним, ігнорується — тож порожня відповідь ніколи не стає хибною.

Вмикай для VP, який годує один принтер із лотками, що ти заряджаєш свідомо; лишай вимкненим для VP, який годує ферму, — там саме вибір з боку BamDude і є сенсом.
