...
| `archive/` | Файли друку й мініатюри, сховище менеджера файлів та вкладення продуктів усередині каталогу архівів. |
| `virtual_printer/` | Робочі файли віртуальних принтерів. |
| `plate_calibration/` | Еталонні кадри перевірки столу. |
| `icons/` | Власні іконки. |
| `projects/` | Вкладення проєктів, що зберігаються тут. |
...
3. BamDude перевіряє базу перед зупинкою фонових сервісів. Пошкоджена база, відсутні основні таблиці чи невідомі версії міграцій відхиляються до зміни робочої БД або ключа шифрування.
4. Відновлення замінює БД та включені до ZIP каталоги й файли з переліку вище. Виконуються лише нові міграції; записані в бекапі не запускаються повторно.
...
На PostgreSQL заміна схеми, завантаження рядків, відновлення лічильників ID та зовнішніх ключів виконуються однією транзакцією БД. Помилка на цих кроках відкочує базу. Якщо заміна БД не вдалася після запису файлу ключа шифрування, повертається і старий файл ключа. Подальші міграції та копіювання каталогів — окремі кроки: відновлення цілого ZIP не є однією транзакцією над БД та файловою системою.
