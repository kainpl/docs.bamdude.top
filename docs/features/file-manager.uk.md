...
Позначте потрібні файли й натисніть **У чергу**. Кнопка зʼявляється, лише коли у вибірці є щось нарізане — STL надрукувати не можна, тож і пропонувати нічого.

Діалог показує обране. Мультиплейтні файли розгортаються плитами, усі позначені; зніміть непотрібні. Ненарізані показані сірим із причиною й участі не беруть.

Далі — куди:

| Призначення | Що стається |
|---|---|
| **Принтери → на кожен** | Копія кожного файла на кожному обраному принтері. Пʼять файлів на трьох принтерах — пʼятнадцять друків. |
| **Принтери → розкидати** | Файли розкладаються по колу між обраними принтерами. Пʼять файлів на трьох принтерах — пʼять друків. |
| **Авто-черга** | Партія йде маршрутизатору, який обере принтер пізніше — коли той справді звільниться і буде видно, що в нього заряджено. |

Розкидання й авто-чергу легко сплутати. Розкидання прибиває кожен друк до конкретного принтера **зараз**; авто-черга вирішує пізніше. Розкидання — коли потрібен передбачуваний розподіл, авто-черга — коли найкращий.

**Кнопка каже, скільки друків створить**, а не скільки файлів ви обрали — три файли можуть бути пʼятнадцятьма друками. Перевіряйте це число до натискання.

Файл, нарізаний під модель принтера, яка його не запустить, звітується, а не ставиться в чергу. При розкиданні такий принтер пропускається й береться наступний, тож ви все одно отримуєте стільки друків, скільки просили.
