...
Коли нічого не збігається, порожній вигляд пропонує **Clear filters** — і скидає всі пʼять: пошук, тип файла, **Не друковані**, ряд чіпів тегів і поле «фільтр за користувачем», плюс наскрізний [фільтр тегів](#чіп-фільтр-тегів), який застосовує сервер.

Останній важливий окремо. Оскільки його застосовує сервер, тег-фільтр, що нічого не збігає, робить порожньою саму бібліотеку — і скид тепер показується й для цього випадку, а не пропозиція завантажити файли.

Ряд чіпів тегів памʼятається між візитами, тож скид тут чистить і збережене — воно не повернеться при наступному перезавантаженні.
