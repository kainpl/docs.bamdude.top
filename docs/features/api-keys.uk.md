...
| Поле | Значення |
|---|---|
| **Префікс** | `bd_` (літерально). Ключі, створені до цієї зміни, починаються з `bb_` — префікса Bambuddy — і працюють далі без змін |
...
| **Printer scope** | Опціонально. Залиш порожнім для "усіх принтерів", або обери конкретні printer ID, щоб звузити ключ. Запит, що називає будь-який інший принтер, отримує 403 — в адресі, у query чи в тілі запиту, а також через те, що належить принтеру (його чергу й задачі в ній, розетку, пункт обслуговування). Списки принтерів, статусів, черг і задач показують лише принтери ключа. Автоматична черга сама обирає принтер, тож ключ з обмеженням за принтерами нею користуватися не може |
| **Expires at** | Опційний ISO timestamp. Після нього ключ відкидається, навіть якщо не ревокнутий |

Відповідь на створення містить поле `key` — **скопіюй його до закриття діалогу**. Подальші читання рядка показуватимуть тільки `bd_…` префікс.
...
# Authorization: Bearer — працює, бо сервер бачить префікс ключа
# і йде по API-key-валідатору замість JWT-валідації.
curl -H "Authorization: Bearer bd_..." http://localhost:8000/api/v1/printers/
```

Обидва шляхи доходять до того самого коду. Префікс `bd_` на `Bearer` токені — або `bb_` на ключі, створеному до зміни префікса, — каже BamDude що це API key, не сесійний JWT, тож JWT-signature path пропускається — запускається key-hash порівняння.
...
??? question "401 Unauthorized — `API key required`"
    Нема ні `X-API-Key`, ні `Authorization` хедера. Додай один з них. Якщо за проксі, що зрізає кастомні хедери — переключайсь на `Authorization: Bearer bd_…`.
