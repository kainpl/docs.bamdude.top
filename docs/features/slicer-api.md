...
A **preset-source control** above the preset dropdowns wires the modal's resolution mode in one place. When no `.bbscfg` bundles have been imported, it renders as the standalone 3-state segmented owner filter (All / My presets / Built-in). When at least one bundle is imported, a top-level **Manual / Bundle** segmented control appears above the sub-control: in **Manual** mode the sub-control is the same 3-state owner filter (classifying cloud presets as custom-vs-builtin via the same `setting_id` regex Profiles uses, `^(P[FPM]US|PF\d|PP\d)`; local imports are always custom, standard bundles are always built-in); in **Bundle** mode the sub-control is the bundle dropdown described below. Persisted in localStorage (owner filter under `bamdude:slice-modal:filter-owner`, last-picked bundle under `bamdude:slice-modal:last-bundle-id`). Switching the owner filter clears any current dropdown selection that no longer matches so a hidden (filtered-out) preset can't silently submit at slice time. Switching to Bundle picks the last-used bundle if it's still in the current list, otherwise the first available bundle (never restores a stale id).
...
**Use in the Slice modal** — when at least one bundle is imported, the preset-source control's **Manual / Bundle** toggle becomes available. Click **Bundle** and the modal flips to bundle-scoped pickers: the printer renders as a read-only label (each `.bbscfg` carries exactly one printer preset, so picking is meaningless), and process + per-slot filament dropdowns are populated from the chosen bundle's contents only — global cloud / local / standard presets are hidden in this mode. Submit routes through `SliceRequest.bundle` and the sidecar materialises the JSON triplet from the stored bundle by name. To leave bundle mode, click **Manual** in the same toggle — `selectedBundleId` clears and the original three-tier dropdowns return.

**Sidecar versioning** — bundle endpoints ship on the BamDude fork's `bamdude/profile-resolver` branch (cherry-picked from `maziggy/orca-slicer-api`'s `bambuddy/bundle-import` upstream branch and rebuilt into our `bamdude-orca-slicer-api:orca2.3.2` + `bamdude-bambu-studio-api:bambu02.06.00.51` images), so a `docker compose --profile all build --pull` from `slicer-api/` pulls fresh sidecars with the routes wired up. The Slice modal's bundle Manual / Bundle toggle silently doesn't render when `GET /api/v1/slicer/bundles` returns `[]` (sidecar offline, no bundles imported yet, or the sidecar pre-dates bundle support) — installs without bundles see exactly the original three-tier modal with just the owner-filter row at the top.
