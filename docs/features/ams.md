...
**Inlet badges:** behind the switch an AMS reaches both nozzles, so instead of a nozzle it is badged with the switch **inlet** it is plumbed into — the one set on the printer's Manual AMS Setup screen. The badge reads **A** or **B**, the inlet's own letter as printed on the switch, in blue rather than the green nozzle badge. It is never **L** or **R**: an inlet can feed either nozzle, and only the switch's outlets are fixed (OUT-A to the left nozzle, OUT-B to the right). The print dialog's slot list carries the same `[IN-A]` / `[IN-B]`. An AMS the switch has not been set up for shows no badge rather than a guess. Changes on the printer screen reach the page live.

**Recommended arrangement:** an inlet is one tube from the AMS through the switch, so two filaments on the same inlet cannot both be in it — loading one pulls the other all the way back to its AMS, even out of the other nozzle. On printers with AMS preload, the outgoing filament stops at the switch and the next one is fed up in the meantime, but only across the two inlets. The slicer works out which filaments belong behind which inlet and stores it in the sliced file; the print dialog compares that with where your spools are, times the difference the way Bambu Studio does, and — only when a move saves at least a second — says which filament to move to an AMS on which inlet and how much time it saves. A print that runs on one nozzle usually needs no split at all, and then the dialog says nothing. It is advice, never a block. Files sliced without this data (older Bambu Studio, other slicers) get no advice.
