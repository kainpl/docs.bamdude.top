...
- **Process profile** — ті самі чотири tier'и, але **відфільтровані під обраний принтер**: профілі для інших принтерів зсуваються в кінцеву групу **«Other printers»** замість того, щоб зникати, а профілі без інформації про принтер лишаються в основному списку (ніколи не ховаються). За замовчуванням — process, під який готувався 3MF, якщо він сумісний; інакше сумісний, найближчий до висоти шару 0.20 мм («Standard» самого слайсера), а не перший за абеткою — який у найменуванні Bambu завжди найтонший і найповільніший. Зміна принтера перевибирає process, якщо поточний більше не підходить.
- **Filament profile(s)** — один dropdown на кожен слот філаменту, **який декларує проєкт**, а не лише на ті, якими малює ця плита; фільтрація та сама (сумісні профілі спершу, група «Other printers» в кінці). Modal pre-pick'ає найкращий match per-slot за filament-metadata вихідного 3MF: профіль **базового матеріалу** слоту (PLA, PETG, ABS… — тип, який заявляє профіль, а не конкретний бренд чи родина профілів; група PA-CF / PA12-CF / PAHT-CF рахується як одна, як і під час друку), точний тип попереду еквівалентного, колір розв'язує нічию, несумісні з принтером — нижче. Профіль іншого матеріалу обирається лише коли більше нічого немає, а профіль, який обрав **ти** — вручну чи з пайплайна, — ніколи не замінюється через матеріал, тож нарізати модель із позначкою PLA у PETG — твоє рішення. Один клік **Slice** зазвичай робить правильне для multi-color jobs. Біля кожного dropdown стоїть контрол **кольору** — колір, яким друкується цей слот; він записується в нарізаний файл, тож його мініатюра й діалог друку показують саме його. Початково це колір, яким плиту задумав дизайнер, або типовий зелений слайсера для джерела без кольору (STL); постав колір котушки, яку заправиш. Жоден слайсер не зберігає колір у профілі філаменту, тож без цього кожен нарізаний файл виходив зеленим Bambu. Це запис, а не вимога: діалог друку й далі дає зіставити слот із котушкою будь-якого кольору.
...
|---|---|:---:|
| **Задум дизайну** | кількість стінок, щільність і патерн заповнення, підтримки, шов, брим, ironing | :material-check: позначено |
| **Визначає пресет** — бейдж `перебиває пресет` | висота шару, висота першого шару | :material-close: не позначено |
...
Висота шару й першого шару — це *і є* вибраний пресет процесу («0.08mm High Quality» — його висота
шару), тож значення з файлу пропонується поряд зі значенням пресету, але ніколи не позначається за тебе:
раніше 0.2 з файлу тихо нарізалось поверх явно вибраного пресету 0.08.
...
**Підтримки слідують за галочками.** Файл із увімкненими підтримками вмикає їх
і для твоєї нарізки — це задум дизайну, позначений за замовчуванням. Зніми
галочки — і нарізка лишить налаштування підтримок твого пресету; знятий параметр
підтримок більше не переноситься потай. Файл, що не записує жодних відхилень
(скажімо, експорт з OrcaSlicer), не має чого позначати, і його підтримки
переносяться, як і раніше.

Ті самі галочки є біля кожного параметра в панелі **Process settings**, і галочка
в будь-якому з двох місць — це одна галочка. Панель сірить параметр лише тоді,
коли його вимкне сама нарізка — за значеннями твого пресету, позначеними
значеннями з файлу і тим, що ти ввів, а не за вбудованими дефолтами слайсера,
через які раніше сіріла вся сторінка Support для пресету з увімкненими
підтримками. Галочку «з файлу» на сірому параметрі все одно можна зняти.
