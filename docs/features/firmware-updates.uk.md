...
1. Качає `.bin` з Bambu CDN (або переюзує локальний кеш — див. [Cache](#firmware-cache) нижче).
...
Завантажені firmware-файли кешуються локально для переюзу через принтери та re-upload-и:

```
<DATA_DIR>/firmware/<MODEL>_<VERSION>.bin
```

| Коли кеш допомагає | Деталь |
|---|---|
| Bulk update тієї ж моделі | П'ять A1 mini на тій самій firmware → один CDN download, п'ять FTP upload-ів |
| Failed upload retry | Повтор upload-у переюзає вже завантажений файл |
| Rollback після зіпсованого upgrade | Попередня firmware скоріше за все ще в кеші |

У кеша нема TTL — файли живуть, поки не видалиш папку руками або не приберешся через on-disk maintenance scripts. Кожен `.bin` 50-150 MB; кеш росте ~1 GB через життя busy install-у.
