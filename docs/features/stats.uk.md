...
**Брак по принтерах** — надруковано, брак і частка по кожній машині за обраний період, найгірша частка зверху; лише завершені друки.
...
Усе, що читає ця сторінка, живе під `/api/v1/statistics/`:

| Endpoint | Що віддає |
|---|---|
| `GET /statistics/overview` | KPI-блок: лічильники, години, грами, вартість, енергія, точність оцінки по принтерах і брак (`date_from`, `date_to`, `created_by_id`) |
| `GET /statistics/aggregate` | Часові серії, розбивки й рекорди, які згортають графіки й календар архівів |
| `GET /statistics/failures` | Аналіз збоїв — див. [Failure analysis](failure-analysis.md) |
| `GET /statistics/export` | Підсумок аналізу збоїв у CSV, або `?format=xlsx` — див. [Export](export.md) |
| `POST /statistics/recalculate-costs` | Перерахувати вартість філаменту всіх архівів за поточними цінами |

Раніші релізи відповідали під `/api/v1/archives/…` (`/archives/stats`, `/archives/aggregate`, `/archives/analysis/failures`, `/archives/stats/export`, `/archives/recalculate-costs`). Ці шляхи зникли, без редиректів. `/api/v1/statistics/reports` зарезервовано під звіти, які будуватимуться тут.
