...
- `WebSocket bootstrap applied`: браузер застосував початкові стани до кешу
  запитів. Зіставляйте `id` із попереднім записом. `client_connect_ms` включає
  отримання токена, підключення й отримання станів; `server_elapsed` також включає
  зворотну доставку підтвердження. Це не вимір першого намальованого кадру.
