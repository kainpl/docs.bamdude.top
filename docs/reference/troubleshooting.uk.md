...
**Діагностика з'єднання** теж бачить цей стан: вона завершує TLS-рукостискання на порту 990 (без входу, тож працює ще до додавання принтера), і відкритий порт, що відхиляє рукостискання, показується окремим попередженням, а не зеленим портом. Поки це триває, BamDude не обходить теки картки в пошуках 3MF — тим спискам потрібне те саме з'єднання, яке щойно відхилили; тригери відновлення архіву повернуться по файл.
